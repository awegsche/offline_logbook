...
copied from Josch's implementation in omc3
"""
import os
import pathlib
...
class RBAC:
    ...
    def authenticate_explicit(self, user: str, password: str, ) -> str:
        ...
        """
        print("Trying to authenticate RBAC via username and password.")
        self._set_user(user)
    ...
    def authenticate_location(self, user: Optional[str] = None) -> str:
        ...
        Returns:
            The RBAC token as a string.
        """
        print("Trying to authenticate RBAC from location.")
        self._set_user(user)
        response = requests.post(
    ...
    def authenticate_kerberos(self) -> str:
        ...
        Returns:
            The RBAC token as a string.
        """
        print("Trying to authenticate RBAC from Kerberos.")
        ...
        print("Starting Kerberos authentication.")
        ...
        print("Received Kerberos ticket.")

        self._set_user()
        print("Sending Kerberos-Login data to RBAC.")
        response = requests.post(
        ...
        print("Cleaning up Kerberos-Client.")
...
def get_os_username():
    ...
    try:
        ...
        print(f"Could not get {RBAC_USERNAME} from environment.")

    try:
        return os.getlogin()
    except OSError as e:
        print(f"Could not get username from login. {str(e)}")

    for variable in ["LOGNAME", "USERNAME", "USER"]:
        try:
            return os.environ[variable]
        except KeyError:
            print(f"Could not get {variable} from environment.")
